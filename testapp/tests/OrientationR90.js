...
class OrientationR90 extends AbstractTest {
    ...
    async testBody(progressCallback) {
        ...
        var outputFile = RNFetchBlob.fs.dirs.DocumentDir + '/output_' + OrientationR90.displayName + '.mp4';
        try {await RNFetchBlob.fs.unlink(outputFile)}catch(e){};

        const status = await Transcode.start()
                .asset({name: "A", path: R90})
                .asset({name: "B", path: R180})
                .asset({name: "C", path: R270})
                .asset({name: "D", path: R0})
            .segment(1000)
                .track({asset: "A"})
            .segment(1000)
...
OrientationR90.displayName = 'OrientationR90';

module.exports = OrientationR90;
